...
unsigned long PackRGBA(unsigned char r, unsigned char g, unsigned char b, unsigned char a) {
    // little-endian bytewise rgba as opposed to RGBA packed which is ABGR bytewise
    ...
    for (int j = 0; j < 4; j++) {
        for (int i = 0; i < 4; i++) {
            ...
            if (alphaCode == 0) {
                ...
                if (alpha0 > alpha1) {
                    finalAlpha = static_cast<unsigned char>(((8 - alphaCode) * alpha0 + (alphaCode - 1) * alpha1) / 7);
                } else {
                    if (alphaCode == 6) {
                        finalAlpha = 0;
                    } else if (alphaCode == 7) {
                        ...
                        finalAlpha = static_cast<unsigned char>(((6 - alphaCode) * alpha0 + (alphaCode - 1) * alpha1) / 5);
                    }
                }
            }
